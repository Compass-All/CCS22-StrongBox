...
// From Rodinia benchmark
// https://github.com/yuhc/gpu-rodinia/blob/master/opencl/lud/ocl/lud.cpp
// 
...
// ========== //
	...
	aes_enc(m, matrix_dim*matrix_dim*sizeof(float));
// ========== //
	...
	cl_mem d_m;
	d_m = clCreateAlignBuffer(context, CL_MEM_READ_WRITE, matrix_dim*matrix_dim * sizeof(float), NULL, &err );
	if(err != CL_SUCCESS) { printf("ERROR: clCreateAlignBuffer d_m (size:%lu) => %d\n", matrix_dim*matrix_dim, err); return -1;} 
#ifdef  TIMING
	...
	// ========== //
	  // Let StrongBox protect and decrypt the GPU buffer [d_m] before the 1-st task execution
	  write_buffer_operation(d_m, i==0, 0);
	  operation_end(cmd_queue);
	// ========== //
	...
	// ========== //	  
	  write_buffer_operation(d_m, 0, 0);
	  operation_end(cmd_queue);
	// ========== //
	...
	// ========== //
	  // Do nothing to d_m since later tasks still use [d_m]
	  write_buffer_operation(d_m, 0, 0);
	  operation_end(cmd_queue);
	// ========== //
...
// ========== //
	// Let StrongBox encrypt [d_m]
	write_buffer_operation(d_m, 0, 1);
	operation_end(cmd_queue);
// ========== //
...
// ========== //
	...
	aes_dec(m, matrix_dim*matrix_dim*sizeof(float));
// ========== //

	FILE *f = fopen("lud_result.txt", "w");
