...
kbase_create_context(struct kbase_device *kbdev, bool is_compat)
{
	...
	if (tagz != 0) {
		...
		err = kbase_mem_pool_init(&kctx->mem_pool,
				...
				&kbdev->s_mem_pool,
				false);
		err = kbase_mem_pool_init(&kctx->page_table_pool,
				kbdev->mem_pool_max_size_default,
				KBASE_MEM_POOL_4KB_PAGE_TABLE_ORDER,
				kctx->kbdev,
				&kbdev->page_table_pool,
				true);
		if (err)
			goto free_kctx;
...
free_mem_pool:
	...
	if (tagz) kbase_mem_pool_term(&kctx->page_table_pool);
free_kctx:
	kctx_addr = NULL;
	vfree(kctx);
out:
...
void kbase_destroy_context(struct kbase_context *kctx)
{
	...
	if (kctx_addr && kctx_addr == kctx) {
		kbase_mem_pool_term(&kctx->page_table_pool);
		kctx_addr = 0;
		secure_pid = -123;
	}
