...
struct page *kbase_mem_alloc_page(struct kbase_mem_pool *pool)
{
	...
	int tagz = 0;
	if (kctx_addr != NULL) {
		struct kbase_context *kctx = (struct kbase_context*)kctx_addr;
		tagz = ( 
			(secure_pid == current->pid) || 
			(pool == &kctx->mem_pool) || 
			(pool == &pool->kbdev->s_mem_pool) || 
			(pool->is_page_table) 
		);
	}

    if (tagz != 0) {
		if (pool->is_page_table) {
			p = cma_alloc(&cma_areas[0], 1 << pool->order, 1 << PAGE_SHIFT, gfp);
		} else
		{
			p = cma_alloc(&cma_areas[1], 1 << pool->order, 1 << PAGE_SHIFT, gfp);
		}
    }
    else {
		...
		struct page *p)
{
	...
	extern int secure_pid;
	int tagz = (current->pid == secure_pid);

	if (tagz) {
		cma_release(&cma_areas[0], p, 1 << pool->order);
	}
	else {
		// cma_release(&cma_areas[1], p, 1 << pool->order);
		__free_pages(p, pool->order);
	}
...
)
{
	...
	pool->dying = false;
	pool->is_page_table = is_page_table;
