...
bool jd_submit_atom(struct kbase_context *kctx, const struct base_jd_atom_v2 *user_atom, struct kbase_jd_atom *katom)
{
	...
	if (kctx && kctx_addr == kctx) {
		// 0: KBASE_JS_ATOM_SCHED_PRIO_MED 
		// 1: KBASE_JS_ATOM_SCHED_PRIO_HIGH 
		// 2: KBASE_JS_ATOM_SCHED_PRIO_LOW
		sched_prio = kbasep_js_atom_prio_to_sched_prio(1); 
	}
	else {
