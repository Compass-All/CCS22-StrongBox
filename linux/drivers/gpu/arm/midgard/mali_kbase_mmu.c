		...
		struct kbase_sub_alloc **prealloc_sas)
{
	...
	if (new_pages >= (SZ_2M / SZ_4K)) {
		...
		*grow_2mb_pool = true;
		...
		if (kctx_addr && kctx_addr == kctx) root_pool = &kctx->page_table_pool;
    	else root_pool = &kctx->mem_pool;
		*grow_2mb_pool = false;
#ifdef CONFIG_MALI_2MB_ALLOC
	}
#endif
...
page_fault_retry:
	...
	if (grown) {
		...
		extern void* kctx_addr;
		if (kctx_addr && kctx_addr == kctx) 
			ret = kbase_mem_pool_grow(&kctx->page_table_pool,pages_to_grow);
		else 
			ret = kbase_mem_pool_grow(&kctx->mem_pool,pages_to_grow);
		...
		struct kbase_mmu_table *mmut)
{
	...
	extern void* kctx_addr;
	int tagz = (kctx_addr && kctx_addr == mmut->kctx);
	if (tagz) 
		p = kbase_mem_pool_alloc(&kbdev->page_table_pool);
	else 
...
alloc_free:
	if (tagz) kbase_mem_pool_free(&kbdev->page_table_pool, p, false);
	else kbase_mem_pool_free(&kbdev->mem_pool, p, false);
					...
					unsigned long flags)
{
	...
	while (remain) {
		...
		do {
			...
			extern void* kctx_addr;
			if (kctx_addr && kctx_addr == kctx) 
				err = kbase_mem_pool_grow(&kctx->kbdev->page_table_pool, MIDGARD_MMU_BOTTOMLEVEL);
    		else 
				err = kbase_mem_pool_grow(&kctx->kbdev->mem_pool, MIDGARD_MMU_BOTTOMLEVEL);
		...
		struct kbase_mmu_table *mmut, u64 *pte)
{
	...
	extern void* kctx_addr;
	if (kctx_addr && kctx_addr == mmut->kctx) 
		kbase_mem_pool_free(&kbdev->page_table_pool, tmp_p, false);
    else 
				    ...
				    unsigned long flags)
{
	...
	while (remain) {
		...
		do {
			...
			int tagz = (kctx_addr && kctx_addr == mmut->kctx);
			if (tagz)
				err = kbase_mem_pool_grow(&kbdev->page_table_pool,
					cur_level);
			else
		...
		int level, u64 *pgd_page_buffer)
{
	...
	extern void* kctx_addr;
	if (kctx_addr && kctx_addr == mmut->kctx) 
		kbase_mem_pool_free(&kbdev->page_table_pool, p, true);
    else 
		...
		struct kbase_context *kctx)
{
	...
	while (!mmut->pgd) {
		...
		extern void* kctx_addr;
		int tagz = (kctx_addr && kctx_addr == kctx);
		if (tagz) 
			err = kbase_mem_pool_grow(&kbdev->page_table_pool,
				MIDGARD_MMU_BOTTOMLEVEL);
		else
