...
void kbase_backend_slot_update(struct kbase_device *kbdev)
{
	...
	for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
		...
		for (idx = 0; idx < SLOT_RB_SIZE; idx++) {
			...
			case KBASE_ATOM_GPU_RB_READY:
				...
				/* If stop_submit equals to 1, then stop submission to GPU hardware. */
				if (kctx_addr && stop_submit) { // existing secure task and secure task is going on
					if (kctx_addr != katom[idx]->kctx) { // block the non-secure task
						break;
					}
					else { // for secure task, check whether other secure task is running in GPU
						int js, break_submit = 0;
						struct kbase_jd_atom *katom[2];
						for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
							katom[0] = kbase_gpu_inspect(kbdev, js, 0);
							katom[1] = kbase_gpu_inspect(kbdev, js, 1);
							if ((katom[0] && katom[0]->kctx == kctx_addr && katom[0]->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED) || 
								(katom[1] && katom[1]->kctx == kctx_addr && katom[1]->gpu_rb_state == KBASE_ATOM_GPU_RB_SUBMITTED)) { 
									break_submit = 1; break;
							}
						}
						if (break_submit) break;
					}
				}
	...
	if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_8987)) {
		//my_printk("kbase_hw_has_issue: BASE_HW_ISSUE_8987");
				...
				struct kbase_jd_atom *katom)
{
	...
	/* Checking the enqueued atom is secure atom or not. If yes, enable stop submit. */
    
	extern void* kctx_addr;
	if (katom->kctx!=NULL && katom->kctx == kctx_addr) {
		stop_submit = 1;
	}
				...
				ktime_t *end_timestamp)
{
	...
	if (completion_code == BASE_JD_EVENT_STOPPED) {
		...
		// extern void *kctx_addr;
		// if (kctx_addr == kctx) {
		// 	stop_submit = 0; // disable stop_submmit.
		// }
...
{
	extern void* kctx_addr;
	//if (katom->kctx == kctx_addr) printk("+++++ kbase_backend_complete_wq() +++++\n");
	/*
