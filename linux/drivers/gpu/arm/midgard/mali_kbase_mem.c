...
int kbase_mem_init(struct kbase_device *kbdev)
{
	...
	ret = kbase_mem_pool_init(&kbdev->s_mem_pool,
			KBASE_MEM_POOL_MAX_SIZE_KBDEV * 4,
			KBASE_MEM_POOL_4KB_PAGE_TABLE_ORDER,
			kbdev,
			NULL,
			false);
	...
	ret = kbase_mem_pool_init(&kbdev->page_table_pool,
			KBASE_MEM_POOL_MAX_SIZE_KBDEV,
			KBASE_MEM_POOL_4KB_PAGE_TABLE_ORDER,
			kbdev,
			NULL,
...
void kbase_mem_term(struct kbase_device *kbdev)
{
	...
	kbase_mem_pool_term(&kbdev->mem_pool);
	kbase_mem_pool_term(&kbdev->s_mem_pool);
	kbase_mem_pool_term(&kbdev->page_table_pool);
