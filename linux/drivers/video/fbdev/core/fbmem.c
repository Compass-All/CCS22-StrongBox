...
__releases(&info->lock)
{
	...
	// printk("[fb_info] smem_start = 0x%lx smem_len = 0x%lx\n", info->fix.smem_start, info->fix.smem_len);
	if (!info) {
...
static int do_register_framebuffer(struct fb_info *fb_info)
{
	...
	if (fb_info->pixmap.addr == NULL) {
		// struct page *p = cma_alloc(&cma_areas[1], 2, 1 << PAGE_SHIFT, GFP_KERNEL);
		// fb_info->pixmap.addr = kmap(p);
		// fb_info->p = p;
		fb_info->pixmap.addr = kmalloc(FBPIXMAPSIZE, GFP_KERNEL);
		// printk("[fb_info] alloc pixmap.addr = 0x%lx\n", fb_info->pixmap.addr);
...
static int do_unregister_framebuffer(struct fb_info *fb_info)
{
	...
	if (fb_info->pixmap.addr &&
	    (fb_info->pixmap.flags & FB_PIXMAP_DEFAULT)) {
			// kunmap(fb_info->p);
			// cma_release(&cma_areas[1], fb_info->p, 2);
			kfree(fb_info->pixmap.addr);
			// printk("[fb_info] free pixmap.addr = 0x%lx\n", fb_info->pixmap.addr);
		}
