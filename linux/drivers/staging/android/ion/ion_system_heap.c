...
static int ion_system_heap_allocate(struct ion_heap *heap,
				    ...
				    unsigned long flags)
{
	// printk("[ION_SYSTEM] ion_system_heap_allocate size: %lu", size);
	...
	while (size_remaining > 0) {
		page = alloc_largest_available(sys_heap, buffer, size_remaining,
					       max_order);
		// printk("[ION_SYSTEM] ion_system_heap_allocate addr: 0x%lx align_size: %lu", page_to_phys(page), PAGE_ALIGN(size));
		if (!page)
			goto free_pages;
...
{
	// printk("[ION_SYSTEM] ion_system_heap_free");
...
static int ion_system_contig_heap_allocate(struct ion_heap *heap,
					   ...
					   unsigned long flags)
{
	// printk("[ION_SYSTEM_CONTIG] ion_system_contig_heap_allocate");
...
{
	// printk("[ION_SYSTEM_CONTIG] ion_system_contig_heap_free");
	struct sg_table *table = buffer->sg_table;
...
{
	// printk("[ION_SYSTEM_CONTIG] ion_system_contig_heap_create");
	struct ion_heap *heap;
